...
class LQGTDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        if self.data_type == 'lmdb' and (self.GT_env is None or self.LQ_env is None):
            self._init_lmdb()
        elif self.data_type == 'lmdb' and ((index+1)%32000==0): # for memory usage consideration
            self.GT_env.close()
            self.LQ_env.close()
            self._init_lmdb()
        ...
        img_GT = util.read_img(self.GT_env, GT_path, resolution, self.opt['YUV'])
        ...
        if self.paths_LQ:
            ...
            img_LQ = util.read_img(self.LQ_env, LQ_path, resolution, self.opt['YUV'])

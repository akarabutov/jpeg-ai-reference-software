...
###################### get image path list ######################
IMG_EXTENSIONS = ['.jpg', '.JPG', '.jpeg', '.JPEG', '.png', '.PNG', '.ppm', '.PPM', '.bmp', '.BMP', '.yuv']
...
def _read_10bit_lmdb(env, key, size):
    """read image from lmdb with key (w/ and w/o fixed size)
    size: (C, H, W) tuple"""
    with env.begin(write=False) as txn:
        buf = txn.get(key.encode('ascii'))
...
def read_img(env, path, size=None, YUV=False):
    """read image by cv2 or from lmdb
    return: Numpy float32, HWC, BGR, [0,1]"""
    if env is None:  #no lmdb file
        if YUV==420: #yuv420
            img = np.squeeze(readyuv420_dataset(path, size))
            if '10bit' in path:
                img = img.astype(np.float32) / 1023.
            else:
                img = img.astype(np.float32) / 255.
        elif YUV==444: #yuv444
            img = np.squeeze(readyuv444_dataset(path, size))
            if '10bit' in path:
                img = img.astype(np.float32) / 1023.
            else:
                img = img.astype(np.float32) / 255.
            ...
            img = img.astype(np.float32) / 255.
    elif (YUV==420 or YUV==444) and path.split('_')[3] == '10bit': # lmdb YUV 10bit
        img = _read_10bit_lmdb(env, path, size)
        img = img.astype(np.float32) / 1023.
    else: # lmdb 8bit
...
def readyuv420_dataset(filename, size = None, startframe=0, totalframe=1):
    #BasketballDrill_832x480_50fps_8bit_420_00.yuv
    #0.002_BasketballDrill_832x480_50fps_8bit_420_00.yuv
    #BasketballDrill_832x480_50fps_8bit_420_00_s000.yuv

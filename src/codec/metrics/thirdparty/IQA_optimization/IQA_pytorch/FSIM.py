...
def phasecong2(im):
    ...
    if OLD_TORCH_VERSION:
        ...
        # torch.fft.fft2(im) doesnt give correct shape 
        imagefft = torch.view_as_real(torch.fft.fft(torch.fft.fft(im, dim=-2), dim=-1))
    ...
    for o in np.arange(norient):
        ...
        for s in np.arange(nscale):
            ...
            if OLD_TORCH_VERSION:
                ...
                # torch.fft.fft2(im) doesnt give correct shape 
                tmp = torch.view_as_complex(imagefft * filter.unsqueeze(-1).repeat(1,1,1,1,2))
                tmp = torch.fft.ifft(torch.fft.ifft(tmp, dim=-2), dim=-1)
                MatrixEO = torch.view_as_real(tmp)

...
class BDRateReporter:
    ...
    def add_summary(self, results_dir: str, epoch: int, stage: str, summaries_list: List) -> None:
        ...
        if os.path.isfile(summary_file):
            ...
            for metric in Config.all_metrics:
                summary_data[metric] = summary_data[metric].astype('float')
    ...
    def calc_bd_rates(self, anchor_vvc_data: pd.DataFrame,
        ...
        """
        anchor_vvc_data_beta_6_to_50 = anchor_vvc_data[(anchor_vvc_data.beta >= 6)
                                                       & (anchor_vvc_data.beta <= 50)]
        anchor_vvc_data_pv = anchor_vvc_data_beta_6_to_50.pivot_table(index=['image', 'beta'])
        collected_summary_data_beta_6_to_50 = collected_summary_data[
            (collected_summary_data.beta >= 6) & (collected_summary_data.beta <= 50)]
        collected_summary_data_pv = collected_summary_data_beta_6_to_50.pivot_table(

...
def split_cp(state_data: Dict[str, torch.Tensor], output_fn: str, op_list: List[str], output_base_dir: str, output_op: List[str] = None, verbose: bool = False) -> None:
    ...
    for i, op in enumerate(op_list):
        ...
        for sn, sv in state_data.items():
            ...
            enc_dec = any(enc_dec)
            if op_substr in sn and enc_dec:
                sn_list = sn.split('.')
    ...
    for comp in ['Y', 'UV']:
        ...
        for sn, sv in state_data.items():
            ...
            sn_list = sn.split('.')
            sn_base = sn_list[0]
            if not enc_dec and (sn_base.endswith(cur_ending) or "_" not in sn_base):
...
def main():
    ...
    split_cp(cp_arr, os.path.basename(args.input_fn), args.op_list, args.output_base_dir, output_op, args.verbose)

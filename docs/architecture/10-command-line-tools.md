...
| `make download_train_ds` | `scripts/download_train_ds.sh` → `download_train_ds.py` | Pull, resume and unpack the training and validation datasets from the JPEG AI sFTP server |
...
```bash
python scripts/download_train_ds.py [--parts train scc validation] [--status] [--dry-run]
       [--data-dir DIR] [--archives-dir DIR] [--skip-download] [--skip-extract]
       [--remove-archives] [--force-lists] [--password-file FILE] [--identity KEY]
       [--refresh] [--list-remote] [--accept-host-key] [--retries N]
```

Fetches the training archives over sFTP with `reget`, so an interrupted transfer resumes, and an
archive whose local size already matches the server is skipped. The archives are then unpacked
into the layout `scripts/train.sh` reads — the natural crops flattened into
`data/jpegai_training_random_crop/`, the SCC patches keeping their own directory, the validation
set in `data/jpegai_validation_set/` — and the training file list is generated by walking that
directory, which covers both flat and nested content. The password is taken from
`--password-file`, from `JPEG_AI_SFTP_PASSWORD` or from a prompt, and reaches `sshpass` through
the environment rather than a command line. `--status` reports what is on disk without
connecting; `--dry-run` prints the plan.

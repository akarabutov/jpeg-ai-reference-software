...
`src/codec` is the library. The other four packages are *tasks*: thin front-ends that reuse the
same codec but do something different with it.
...
├── profiles/              simple.json, main.json, high.json + profiles_list.json, levels.json
